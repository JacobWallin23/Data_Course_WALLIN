...
try(vector3 + 1, silent = TRUE)
try(as.character(vector1) + 5, silent = TRUE)
vector3 <- base::letters
vector3_num <- as.integer(factor(vector3, levels = base::letters))


vector3_num + 5
vector2 / 2
stopifnot(is.numeric(vector1), is.numeric(vector2))
vector1*vector2

# vector3 + 1 # can't add 1 to "a"
vector3_num_plus1 <- vector3_num + 1
...
vector3_num2 <- match(vector3, letters)
head(vector3_num2)
...
data("iris")
iris_even <- iris[seq(2, nrow(iris), by = 2), ]
...
Sepal.Area <- with(iris, Sepal.Length * Sepal.Width)
...
cat("big_area_iris rows:", nrow(big_area_iris), "\n")
